...
def play_game(player, entities, game_map, message_log, game_state, con, panel,
    ...
    while not libtcod.console_is_window_closed():
        ...
        render_all(con, panel, entities, player, game_map, fov_map,
                   message_log, fov_recompute, constants['screen_width'],
        ...
        for player_turn_result in player_turn_results:
            ...
            if xp:
                ...
                message_log.add_message(Message(f'You gain {xp} exp points'))

                if leveled_up:
                    message_log.add_message(Message(
                        f'LVLUP! Reached level {player.level.current_level}',
                        libtcod.yellow))
                    previous_game_state = game_state
...
def main():
    ...
    while not libtcod.console_is_window_closed():
        ...
        if show_main_menu:
            ...
            if show_load_error_message and (new_game or load_saved_game or
                                            exit_game):
                show_load_error_message = False
            elif new_game:
