...
def render_all(con, panel, entities, player, game_map, fov_map,
    ...
    if game_state in (GameStates.SHOW_INVENTORY, GameStates.DROP_INVENTORY):
        if game_state == GameStates.SHOW_INVENTORY:
            inventory_title = ('Press the key next to an item to use it, or '
                               'Esc to cancel\n')
        else:
            inventory_title = ('Press the key next to an item to drop it, or '
                               'Esc to cancel\n')
        inventory_menu(con, inventory_title, player.inventory, 50,
                       screen_width, screen_height)
    elif game_state == GameStates.LEVEL_UP:
        level_up_menu(con, 'LVL UP! Choose a stat to raise:', player, 40,
                      screen_width, screen_height)
